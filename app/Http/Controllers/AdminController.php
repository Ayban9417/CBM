...
class AdminController extends Controller
{
    ...
    public function getParentId($username){
        ...
        $parent = DB::table('users')->where('username', $username)
        ->where('activated', '=', 'yes')
    ...
    public function activateUser(Request $request)
    {
        ...
        try {
         ...
         if($node_id == 1) {
            ...
            $parent = DB::table('users')->where('username', $request->by)->first();
             ...
             $this->pay($parent, $this->level1Payment, $type);

            $parent_id =  $this->getParentId($parent->username);

           
            // $fparent = DB::table('users')
            // ->select(DB::raw('users.*, users_tree.*'))
            // ->where('users_tree.descendant', '=', $parent_id)
            // ->where('users_tree.depth', '=',0)
            // ->join('users_tree', 'users.id', '=', 'users_tree.ancestor')
            // //->where('users.level', '>=', $level)
            // ->first();
            ...
            //     $type = "Direct Referral";
            //     $this->pay($fparent, $this->level1Payment, $type);  
        ...
        if($parent_id != 0) {
           // $realParent_id = 0;
            ...
            $realParent_id = $this->getParentId($realParent->username);
            // $type = "2nd Level Bonus";

            // $this->pay($realParent, $this->level2Payment, $type);

            // if($getDownlines = 2){
            //     $sec_parent = DB::table('users')->where('users.parent_id', $parent_id)->first();
            //     $type = "2nd Level Referral";
            //     $this->pay($sec_parent, $this->level2Payment, $type);
            // }
           
        }
         if($parent_id > 1){
            $sparent = DB::table('users')
            ->select(DB::raw('users.*, users_tree.*'))
            ->where('users_tree.descendant', '=', $parent_id)
            ->where('users_tree.depth', '=', 1)
            ->join('users_tree', 'users.id', '=', 'users_tree.ancestor')
            //->where('users.level', '>=', $level)
            ->first();
 
                $type = "2nd Level Referral";
                $this->pay($sparent, $this->level2Payment, $type);
                $sparent_id = $this->getParentId($sparent->username);  
                
                if($sparent_id > 1){
                    $tparent = DB::table('users')
                    ->select(DB::raw('users.*, users_tree.*'))
                    ->where('users_tree.descendant', '=', $parent_id)
                    ->where('users_tree.depth', '=', 2)
                    ->join('users_tree', 'users.id', '=', 'users_tree.ancestor')
                    //->where('users.level', '>=', $level)
                    ->first();
        
                   $type = "3rd Level Referral";
                   $this->pay($tparent, $this->level3Payment, $type);
            ...
            //DB::table('users')->where('id', $realParent_id)->get('parent_id');
        ...
        // $ghost =  DB::table('users_tree')->where('ancestor', $realParent_id)->first();

        // $type = "2nd Level Referral";

        // $this->pay($ghost, $this->level2Payment, $type);
