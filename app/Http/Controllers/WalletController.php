    ...
    public function sendPaymentRequest(Request $request){
        ...
        if($wallet = Wallet::where('user_id',  Auth::id())->first()) {
            if($wallet->amount < $request->amount) {
                ...
                if( intval($request->amount) < 500 ) {
                    $request->session()->flash('error', 'Minimum of 500 please!');
                    return redirect()->back();
                }
                ...
                if($wallet->amount < 500) {
                    $request->session()->flash('error', 'You should at least leave Php 500 to your wallet !');
                    return redirect()->back();
                }
                $wallet->save();
