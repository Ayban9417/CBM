                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="myTable" class="table table-bordered table-striped">
                                        ...
                                        @foreach ($users as $user)
                                            <tr>
                                                <td>{{$user->name}}</td>
                                                <td>Php {{ !$user ? 0 : number_format($user->amount) }}</td>
                                               
                                                <td>{{$user->username}}</td>
                                                <td>{{$user->email}}</td>
                                                <td>{{$user->phone}}</td>
                                                <td>{{$user->level}}</td>
                                                <td>Php {{ !$user ? 0 : number_format($user->balance) }}</td>
                                                <td>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                        ...
                                                        @csrf
                                                                ...
                                                                <h5>Withdrawal Option (Walk-in, Gcash):&nbsp;  {{$user->bank_name}}</h5>
                                                                <h5>Account name: &nbsp; {{$user->account_name}}</h5>
                                                                <h5>Phone no:&nbsp; <b> {{$user->account_no}}</b></h5>
